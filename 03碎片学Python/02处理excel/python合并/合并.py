...
time0 = time.time()
# 文件名----------------------
file1 = ['\\2019.12-2020.6铁通包干费用明细.xlsx', '明细表']
file2 = ['\\分光器的配置查询结果.csv', '明细表']
...
if os.path.splitext(file2[0])[-1][1:] == 'csv':
    ...
    df1 = pd.read_excel(path + file2[0], sheet_name=file1[1])
else:
...
df3 = pd.merge(df1, df2.loc[:, ['分光器名称', '创建时间']],
               how='left', left_on=['分光器名称'], right_on=['分光器名称'])
