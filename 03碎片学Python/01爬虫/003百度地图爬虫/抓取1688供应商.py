...
class Chrome_drive():
    ...
    def index_page(self, page, wd):
        ...
        self.browser.switch_to.window(
            self.browser.window_handles[-1])  # 此行代码用来定位当前页面窗口
        self.buffer()  # 网页滑动  成功切换
        # 等待元素加载出来
        time.sleep(3)
        self.wait.until(EC.presence_of_element_located(
            (By.CSS_SELECTOR, '#render-engine-page-container > div > div.common-pagination > div > div > div > span:nth-child(2) > input')))
        # 获取网页的源代码
    ...
    def buffer(self):  # 滑动网页的
        for i in range(20):
            ...
            self.browser.execute_script(
                'window.scrollBy(0,380)', '')  # 向下滑行300像素。
...
def save_csv(lise_line):
    file = csv.writer(open("./1688_com.csv", 'a',
...
def get_products(wd, html_text):
    ...
    for i in range(1, len(items)+1):
        ...
        # 图片链接
        img_url = select.xpath(
            f'//*[@id="sm-offer-list"]/div[{i}]//div[@class="img"]/@style').extract_first()
        # 复购率
        ...
        all_desc = [wd, title_name, img_url, desc_href, price, sales_num,
                    company_name, company_href, company_tag, shop_repurchase_rate]
        print(all_desc)
        save_csv(all_desc)
